...
def fillYCPT(bayesNet, gameState):
    ...
    yFactor.setProbability({Y_POS_VAR: LEFT_BOTTOM_VAL}, 1 - PROB_ONLY_LEFT_TOP)
...
def fillObsCPT(bayesNet, gameState):
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            obsFactor = bn.Factor([obsVar], [housePos, GHOST_HOUSE_VAR, FOOD_HOUSE_VAR], bayesNet.variableDomainsDict())

            obsFactor.setProbability({obsVar: NO_OBS_VAL}, 1.0)
            obsFactor.setProbability({obsVar: RED_OBS_VAL, GHOST_HOUSE_VAR: housePos}, PROB_GHOST_RED)
            obsFactor.setProbability({obsVar: BLUE_OBS_VAL, GHOST_HOUSE_VAR: housePos}, 1 - PROB_GHOST_RED)
            obsFactor.setProbability({obsVar: RED_OBS_VAL, FOOD_HOUSE_VAR: housePos}, PROB_FOOD_RED)
            obsFactor.setProbability({obsVar: BLUE_OBS_VAL, FOOD_HOUSE_VAR: housePos}, 1 - PROB_FOOD_RED)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    foodMarginal = inference.inferenceByVariableElimination(bayesNet, [FOOD_HOUSE_VAR], evidence, eliminationOrder)
    ghostMarginal = inference.inferenceByVariableElimination(bayesNet, [GHOST_HOUSE_VAR], evidence, eliminationOrder)

    foodProbs = foodMarginal.getProbabilityDict()
    ghostProbs = ghostMarginal.getProbabilityDict()

    maxFood = max(foodProbs, key=foodProbs.get)
    maxGhost = max(ghostProbs, key=ghostProbs.get)

    if maxFood == maxGhost:
        return maxFood  # Use food distribution for ties
    else:
        return maxFood
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        # Iterate over both possible house positions (left and right)
        for foodHouseVal in [TOP_LEFT_VAL, TOP_RIGHT_VAL]:
            ghostHouseVal = TOP_RIGHT_VAL if foodHouseVal == TOP_LEFT_VAL else TOP_LEFT_VAL

            # Update the evidence with the current house positions
            houseEvidence = dict(evidence)
            houseEvidence[FOOD_HOUSE_VAR] = foodHouseVal
            houseEvidence[GHOST_HOUSE_VAR] = ghostHouseVal

            # Calculate the expected value of entering the left and right houses
            houseMarginals = inference.inferenceByVariableElimination(self.bayesNet, [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR],
                                                                      houseEvidence, eliminationOrder)
            houseValue = self.computeHouseValue(foodHouseVal, houseEvidence, eliminationOrder)

            if foodHouseVal == TOP_LEFT_VAL:
                leftExpectedValue = houseValue
            else:
                rightExpectedValue = houseValue
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        # Get the list of probabilities and exploration outcomes
        explorationProbsAndOutcomes = self.getExplorationProbsAndOutcomes(evidence)

        # Iterate over the possible exploration outcomes
        for prob, explorationEvidence in explorationProbsAndOutcomes:
            # Calculate the value of entering the house with the highest expected value
            leftExpectedValue, rightExpectedValue = self.computeEnterValues(explorationEvidence, enterEliminationOrder)
            maxExpectedValue = max(leftExpectedValue, rightExpectedValue)

            # Update the expected value by considering the probability of this exploration outcome
            expectedValue += prob * maxExpectedValue
